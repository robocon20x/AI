...
def astar_bonus(start,end,matrix,bonus_points,flag):
    open = [] # hang doi
    close = [] # cac o da chay
    path = {} # luu quan he cha con giua cac o 
    result_path = []
    result_path_temp = []
    # bonus_point = [(bonus_points[i],hx.heuristic(bonus_points[i],end,flag)) for i in range(len(bonus_points))] # tim h_x cho cac bonus_point point
    open.append(start)
    while len(open) > 0:
        
        min = hx.heuristic(open[0],end,flag) +  g_x(start,open[0],path)
        pos = 0
        pos_b = -1
        for i in range(len(open)):
            for j in range(len(bonus_points)):
                distace = bonus_points[j][2] + hx.heuristic(open[i],(bonus_points[j][0],bonus_points[j][1]),flag) + \
                     hx.heuristic((bonus_points[j][0],bonus_points[j][1]),end,flag) #  + g_x(start,open[i],path) diem thuong + khoang cach den diem dang xet + khoang cach tu bonus_point[i] den end
                if distace < min:
                    min = distace
                    pos = i
                    pos_b = j
        if pos_b == -1:
            for i in range(len(open)):
                if hx.heuristic(open[i],end,flag) + g_x(start,open[i],path) < min:
                    min = hx.heuristic(open[i],end,flag) + g_x(start,open[i],path)
                    pos = i 
        ...
        if pos_b != -1:
            if curCell == (bonus_points[pos_b][0],bonus_points[pos_b][1]):
                ...
                result_path_temp = astar_bonus(curCell,end,matrix,bonus_points,flag)
                break
